...
buttons = []
GAME_MODE = "human_vs_agent"  # options: "human_vs_agent", "human_vs_human", "agent_vs_agent"

current_player = 0  # 0 = Player A (human), 1 = Player B (agent or human)
game_over = False

agentA = None
agentB = Agent((state.rows, state.cols), "AI") if GAME_MODE == "human_vs_agent" else None
...
def update_button_color(r, c):
    val = state.grid[r][c]
    if val == 0:
        buttons[r][c].config(bg=EMPTY_COLOUR)
    elif val == 1:
        buttons[r][c].config(bg=COUNTER_COLOUR)
    elif val == 2:
        buttons[r][c].config(bg=HINGER_COLOUR)
...
def on_click(r, c):
    ...
    # Human move
    if GAME_MODE in ["human_vs_agent", "human_vs_human"] and current_player == 0:
        if not IsValidMove(state, r, c):
            print("Invalid move!")
            return
        
        WasHinger = IsHinger(state, r, c)
   
        #make move, and update grid
        state.grid[r][c] -= 1
        buttons[r][c].config(text=str(state.grid[r][c]))
        update_button_color(r, c)
        print(f"Button clicked at ({r}, {c}) -> New value : {state.grid[r][c]}")
        print(state)
        if WasHinger:
            show_end_message("Human wins!")
            game_over = True
            return
...
def agent_move():
    ...
    Score, Move = agentB.move(state, "minimax")
    if Move is None:
        show_end_message("No valid move — Human wins!")
        game_over = True
        return
    ...
    buttons[r][c].config(text=str(state.grid[r][c]))
    update_button_color(r, c)
    print(f"Agent moved at {Move}")

    if WasHinger:
        show_end_message("Agent wins!")
        game_over = True
        return

    current_player = 0
    status_label.config(text="Turn: Human")
...
def show_end_message(message):
    ...
    popup.configure(bg="#FFF3B0")

    msg = tk.Label(popup, text=message, font=("Helvetica", 22, "bold"), bg="#FFF3B0")
    msg.pack(expand=True)

    btn = tk.Button(popup, text="Close", command=root.destroy, bg="#FF8FAB")
    btn.pack(pady=20)
